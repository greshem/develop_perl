...
die "can't authenticate"
 unless $ssh2->auth(username => scalar getpwuid($<),  password=>"qian********43", interact => 1);
 #unless $ssh2->auth(username => scalar getpwuid($<), interact => 0);
...
$chan->exec('cat /etc/passwd') or die("exec passwd error \n");;
_read($chan);
